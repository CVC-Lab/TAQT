...
int BufferedIO::get(unsigned int* data, int n)
{
#ifdef _DEBUG
	assert(m_fp);
#endif
	n = getraw(data, sizeof(unsigned int), n);
#ifdef _LITTLE_ENDIAN
	int tmp;
	char *p_tmp = (char*)&tmp;
	char *p_data = (char*)data;
	int tsize = sizeof(unsigned int);
	for(int i = 0; i < n; i++) {
		tmp = data[i];
		// swap byte order
		for(int j = tsize-1; j >= 0; j--) {
...
void BufferedIO::put(const unsigned int* data, int n)
{
#ifdef _DEBUG
	assert(m_fp);
#endif
...
#else
	putraw(data, sizeof(unsigned int)*n);
#endif		// LITTLE_ENDIAN
}
